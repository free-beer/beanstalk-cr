...
describe Beanstalk::JobSettings do
  describe "#initialize()" do
    context "default constructor" do
      it "returns an instance with default settings" do
        settings = Beanstalk::JobSettings.new
        settings.delay.should eq Beanstalk::JobSettings::DEFAULT_DELAY
        settings.priority.should eq Beanstalk::JobSettings::DEFAULT_PRIORITY
        settings.time_to_run.should eq Beanstalk::JobSettings::DEFAULT_TIME_TO_RUN
      end
    ...
    context "explicit constructor" do
      context "when given a single parameter" do
        it "returns an instance with all settings set appropriately" do
          settings = Beanstalk::JobSettings.new(500_u32)
          settings.delay.should eq Beanstalk::JobSettings::DEFAULT_DELAY
      ...
      context "when given two parameters" do
        it "returns an instance with all settings set appropriately" do
          settings = Beanstalk::JobSettings.new(500_u32, 123_u32)
      ...
      context "when given three parameters" do
        it "returns an instance with all settings set appropriately" do
          settings = Beanstalk::JobSettings.new(500_u32, 123_u32, 456_u32)
          settings.delay.should eq 123_u32
          settings.priority.should eq 500_u32
          settings.time_to_run.should eq 456_u32
        end
